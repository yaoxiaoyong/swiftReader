...
static E_ScrollViewController *Instance;
...
-(void)configChapter:(NSString*)title content:(NSString*)content chapterIndex:(NSInteger)index{
    [E_ReaderDataSource shareInstance].currentChapterIndex = index;
    E_EveryChapter *chapter = [[E_ReaderDataSource shareInstance] openChapter:index];
    chapter.chapterTitle = title;
    NSLog(@"chapter.chapterTitle=%@", chapter.chapterTitle);
...
{
    [super viewDidLoad];
    NSArray *imageList = @[[UIImage imageNamed:@"sina.png"],
                           [UIImage imageNamed:@"friend.png"],
                           [UIImage imageNamed:@"weixin.png"],
    ...
    ///need to do adapter
    [E_ReaderDataSource shareInstance].totalChapter = 452;
    [E_ReaderDataSource shareInstance].currentChapterIndex = 0;

    //设置总章节数
    NSLog(@"总章节数:%lu", (unsigned long)[E_ReaderDataSource shareInstance].totalChapter);
    NSLog(@"当前章节数:%lu", (unsigned long)[E_ReaderDataSource shareInstance].currentChapterIndex);
...
- (void)turnToClickMark:(E_Mark *)eMark{
    ...
    NSInteger showPage = [self findOffsetInNewPage:(NSInteger)(NSRangeFromString(eMark.markRange).location)];
      ...
      viewControllerBeforeViewController:(UIViewController *)viewController
{
    ...
    ///need to do adapter
    NSLog(@"go before, 当前页码为%ld", (unsigned long)currentPage);
    //[dataset sharedInstance]->currentNovenCurrentPage = currentPage+2;
//    [MyNotification sendPageChangedNotification:[NSNumber numberWithInteger:currentPage]];
       ...
       viewControllerAfterViewController:(UIViewController *)viewController
{
    ...
    E_ReaderViewController *reader = (E_ReaderViewController *)viewController;
    NSUInteger currentPage = reader.currentPage;
    NSLog(@"go after, currentPage=%lu", (unsigned long)currentPage);
